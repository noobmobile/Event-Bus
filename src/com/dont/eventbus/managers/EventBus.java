...
public class EventBus {
	...
	public void callEvent(Event event) {
		if (invocations.containsKey(event.getClass())) {
			invocations.get(event.getClass()).stream()
			.sorted(Comparator.comparing((Invocation invocation) -> invocation.getEventPriority().getPriority()).reversed()) // ordem crescente, pois prioridades mais altas são executado por último
					.forEach(invocation -> invocation.invoke(event));
		}
	}
	...
	public void registerEvents(Listener listener) {
		...
		methods.forEach(method -> {
			Invocation invocation = new Invocation(method, listener, method.getAnnotation(EventHandler.class).priority());
	...
	private List<Method> getHandlerMethods(Class<? extends Listener> clazz) {
		return Arrays.asList(clazz.getDeclaredMethods()).stream().filter(method -> method.isAnnotationPresent(EventHandler.class)).filter(method -> method.getParameterCount() == 1)
				.filter(method -> Event.class.isAssignableFrom(method.getParameterTypes()[0])).collect(Collectors.toList());
	}
